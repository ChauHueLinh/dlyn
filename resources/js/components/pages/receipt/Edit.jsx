...
export default function Add(props) {
	...
	useEffect(() => {
		let new_products = []
		let total_receipt = 0
		let coupon_receipt = NUMBER.format(0)
		...
		props?.data?.products?.map((item) => {
			...
			new_products.push({ id: item.id, name: item.name, price: item.pivot.price, quantity: item.pivot.quantity, total: item.pivot.total })
		})
		setProducts([...new_products])
		setTotalReceipt(total_receipt)
		if (props?.data?.coupon?.unit == 'VND') {
			coupon_receipt = props?.data?.coupon?.value
		} else if (props?.data?.coupon?.unit == '%') {
			coupon_receipt = Number(total_receipt) * Number(props?.data?.coupon?.value) / 100
		}
	...
	return (
		<Modal
			...
			wrapperClass='w-75'
		>
			<h2 className="text-lg font-medium leading-6 text-gray-900 mb-4"> Chỉnh sửa hóa đơn</h2>
			...
			<div className="flex w-full justify-content-end space-x-6 mb-4 border-top border-dark">
				...
				<div className="w-50">
					...
					<div className="flex space-x-1">
						...
						<div className="w-50 h5 text-end">{VND.format(props?.data?.total)}</div>
					</div>
					<div className="flex space-x-1">
